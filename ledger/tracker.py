...
so the circuit breaker has a real P&L series to gate on. Exits assume an
idealized fill exactly at the stop/target price; real-fill divergence is
a Stage 2 concern (see README Stage 2 notes).
"""
...
class Ledger:
    ...
    def check_exits(self, current_price: float, now: datetime | None = None) -> list[ClosedPosition]:
        """Close any open position whose stop or target is touched by current_price.

        Stop takes precedence if both are somehow touched in the same
        check (conservative assumption). Fill is assumed exactly at the
        stop/target level.
        """
