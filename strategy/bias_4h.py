...
See docs/STRATEGY_PSEUDOCODE.md for the decision tree this implements and
docs/RESEARCH_FINDINGS.md section 3.2-3.4 for cited sources.
"""
...
def detect_swings(candles: Sequence[Candle], fractal_width: int = 2) -> list[Swing]:
    ...
    A candle is confirmed as a swing point once `fractal_width` bars have
    closed after it — this guarantees swings never repaint once formed.
    """
...
def compute_bias(candles: Sequence[Candle], fractal_width: int = 2, sr_lookback: int = 20) -> BiasResult:
    ...
    if last_swing.direction == SwingDirection.UP:
        ...
        if price > fib_levels["0.618"] and (support is None or price > support.price):
            return BiasResult(
                ...
                f"price {price} above 0.618 retrace {fib_levels['0.618']:.2f} and holding support",
            )
    ...
    if price < fib_levels["0.618"] and (resistance is None or price < resistance.price):
        return BiasResult(
            ...
            f"price {price} below 0.618 retrace {fib_levels['0.618']:.2f} and holding resistance",
        )
