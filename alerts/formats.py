...
Per explicit user instruction: this does not import from, and was not
copied out of, Bullphoric's alerts/formats.py. The emoji-header /
labeled-field layout below is a generic, commonly-used shape for a
Telegram trading alert, not a shared implementation.
"""
...
def format_entry_signal(signal: Signal, quantity: float, risk_pct: float, risk_amount: float) -> str:
    ...
    return (
        f"{emoji} BTC-PERP {direction_label} SIGNAL\n"
